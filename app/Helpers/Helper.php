...
if (!function_exists('ActivatedModule')) {
    function ActivatedModule($user_id = null)
    {
        ...
        if (!empty($user)) {
            ...
            // Plans / memberships have been removed: every company (and therefore
            // every sub-user under that company) now has access to all enabled
            // modules without requiring a plan to assign them.
...
// Plans / memberships removed: there is no longer a per-plan seat limit, so
// companies may always create users.
...
// Plans / memberships removed: when a new company is created it must immediately
// have full access to every enabled module. This replaces the old plan-assignment
// onboarding by activating all enabled modules, seeding their default per-company
// data, granting module permissions to the default roles, and setting unlimited
// users/storage.
if (!function_exists('setupCompanyDefaults')) {
    function setupCompanyDefaults($userId)
    {
        ...
        $enabledModules = array_values((new Module())->allEnabled());

        // Activate every enabled module for this company.
        ...
        // Unlimited users and storage (no plan caps anymore).
        $user->active_plan = 1;
        $user->plan_expire_date = null;
        $user->total_user = -1;
        $user->storage_limit = 0; // 0 = unlimited
        $user->save();

        $modules = implode(',', $enabledModules);

        // Let packages seed their default per-company data.
        DefaultData::dispatch($user->id, $modules);

        // Grant the module permissions to the company's default staff/client roles.
        $clientRole = Role::where('name', 'client')->where('created_by', $user->id)->first();
        $staffRole  = Role::where('name', 'staff')->where('created_by', $user->id)->first();
        if (!empty($clientRole)) {
            GivePermissionToRole::dispatch($clientRole->id, 'client', $modules);
        }
        if (!empty($staffRole)) {
            GivePermissionToRole::dispatch($staffRole->id, 'staff', $modules);
        }
    }
}
