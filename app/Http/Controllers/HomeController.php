...
class HomeController extends Controller
{
    ...
    private function superAdminDashboard()
    {
        ...
        // Plans / memberships removed: show new-company sign-ups per month instead
        // of order/payment data.
        $companyData = User::selectRaw('MONTH(created_at) as month, COUNT(*) as count')
            ->where('type', 'company')
        ...
        for ($i = 1; $i <= 12; $i++) {
            $chartData[] = [
                'month' => $months[$i-1],
                'companies' => $companyData[$i]->count ?? 0,
            ];
        }
        ...
        return Inertia::render('SuperAdminDashboard', [
            'stats' => [
                ...
                'total_users' => User::whereNotIn('type', ['superadmin'])->count(),
            ],
            'chartData' => $chartData,
