...
class UserController extends Controller
{
    ...
    public function store(StoreUserRequest $request)
    {
        if(Auth::user()->can('create-users')){
            ...
            // Plans / memberships removed: a company created by the super admin can
            // use the system immediately with all modules active and no plan.
            if(Auth::user()->type == 'superadmin')
            {
                setupCompanyDefaults($user->id);
            }
    ...
    // Plans / memberships removed: plan assignment is no longer available.
    // Kept as a harmless stub in case any old reference calls it.
    public function assignPlan(Request $request, User $user)
    {
        return back()->with('error', __('Plans have been removed from this system.'));
    }
